 ...
 * 6 2 2      n = 6,  c = 2, m = 2 (third test case)
 ...
 Bobby makes the following 3 trips to the store:
  ...
  3. He spends 6 on 3 chocolates at 2 apiece. He then exchanges 2 of the 3 wrappersfor 1 additional piece.
      ...
      for 1 more piece. At this point he has 1 wrapper left, which is not enough to perform another trade-in. He eats 5 chocolates.
 */
public class ChocolateFeast {

    public static int computeNumberOfConsumedChocolates(int n, int c, int m) {
        int wrappers = (int) Math.floor(n/c);
        int totalChocoEaten = (int) Math.floor(n/c);
        while(wrappers >= m) {
            int chocolateEaten = (int) Math.floor(wrappers/m);
            totalChocoEaten+=chocolateEaten;
            wrappers-=chocolateEaten*m;
            wrappers+=chocolateEaten;
        }
        return totalChocoEaten;
    }
}
